...
**Chat sessions (implemented):** Each RAG conversation is saved to the dataset (`chat_sessions` + `chat_messages` in Supabase). "New chat" starts a fresh conversation; past chats appear in the session sidebar and can be reopened or deleted. **One-time setup:** Run `backend/ingestion/schema_chat_sessions.sql` in the Supabase SQL editor to create the tables and RLS. API: POST/GET/DELETE `/api/sessions`, GET `/api/sessions/{id}` (session + messages); GET `/api/chat?q=...&session_id=...` persists each turn when `session_id` is provided.
